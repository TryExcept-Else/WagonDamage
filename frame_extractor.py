...
try:
    torch.serialization.add_safe_globals([
        ...
        nn.SiLU,
        nn.Upsample,
        nn.ModuleList  # Add the newly required ModuleList class
    ])
...
def extract_and_annotate_wagons(video_path, output_dir_frames, output_video_path, model_path):
    """
    Processes a video to detect wagons, saves annotated frames of single wagons,
    and creates an annotated video.

    Args:
        video_path (str): Path to the input video file.
        output_dir_frames (str): Directory to save the extracted wagon frames.
    ...
    try:
        ...
        logger.info(f"Output frames will be saved to: {output_dir_frames}")
            ...
            if current_capture_state == "SEARCHING_FOR_WAGON":
                ...
                if num_current_wagon_boxes == 1:
                    ...
                    if potential_capture_frame_img is not None:
                        filename = f"wagon_capture_{saved_frame_count:03d}_frame{frame_idx-CAPTURE_DELAY}.jpg"
                        filepath = os.path.join(output_dir_frames, filename)
                        cv2.imwrite(filepath, potential_capture_frame_img)
    ...
    if current_capture_state == "SINGLE_WAGON_PASSING" and potential_capture_frame_img is not None:
        filename = f"wagon_capture_{saved_frame_count:03d}_frame{frame_idx-CAPTURE_DELAY}_end.jpg"
        filepath = os.path.join(output_dir_frames, filename)
        cv2.imwrite(filepath, potential_capture_frame_img)
        saved_frame_paths.append(filepath)
        saved_frame_count += 1
    ...
    logger.info(f"Processing complete. Saved {saved_frame_count} individual wagon frames.")
    return saved_frame_count, saved_frame_paths